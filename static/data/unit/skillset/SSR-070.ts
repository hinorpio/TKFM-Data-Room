import { DispatchSkillType, Locale, SkillType } from '@/plugins/utils/enums';
import { SkillSet } from '@/interface/unit/skillset';

export const SkillSet_10106: SkillSet[] =  [
    {
        version: 1,
        lastDate: "",
        remark: {
            [Locale.tc]: ``,
            [Locale.sc]: ``,
            [Locale.en]: ``,
            [Locale.jp]: ``,
            [Locale.kr]: ``
        },
        skill: {
            [Locale.tc]: {
                [SkillType.SKILL_S]: {
                    name: `精靈箭術。連連矢`,
                    description: `使自身造成傷害增加10/10/15/15/20%(3回合)，以自身攻擊力200%對目標造成傷害，再使自身獲得「攻擊時，觸發『以自身攻擊力50/65/80/95/110%對目標造成傷害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_1]: {
                    name: `精靈箭術。連連矢`,
                    description: `使自身造成傷害增加10%(3回合)，以自身攻擊力200%對目標造成傷害，再使自身獲得「攻擊時，觸發『以自身攻擊力50%對目標造成傷害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_2]: {
                    name: `精靈箭術。連連矢`,
                    description: `使自身造成傷害增加10%(3回合)，以自身攻擊力200%對目標造成傷害，再使自身獲得「攻擊時，觸發『以自身攻擊力65%對目標造成傷害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_3]: {
                    name: `精靈箭術。連連矢`,
                    description: `使自身造成傷害增加15%(3回合)，以自身攻擊力200%對目標造成傷害，再使自身獲得「攻擊時，觸發『以自身攻擊力80%對目標造成傷害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_4]: {
                    name: `精靈箭術。連連矢`,
                    description: `使自身造成傷害增加15%(3回合)，以自身攻擊力200%對目標造成傷害，再使自身獲得「攻擊時，觸發『以自身攻擊力95%對目標造成傷害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_5]: {
                    name: `精靈箭術。連連矢`,
                    description: `使自身造成傷害增加20%(3回合)，以自身攻擊力200%對目標造成傷害，再使自身獲得「攻擊時，觸發『以自身攻擊力110%對目標造成傷害』(3回合)」，CD: 3`
                },
                [SkillType.ATTACK]: {
                    name: `拉弦射撃`,
                    description: `以自身攻擊力100%對目標造成傷害`
                },
                [SkillType.LEADER]: {
                    name: `深藏不露的勝負心`,
                    description: `我方全體HP增加20%\n我方全體攻擊者、妨礙者獲得「隊伍中至少有3名風屬性角色時，發動《精靈隊長》」\n我方全體獲得「隊伍中至少有2名火屬性角色時，發動《狙擊陣形》」\n\n《精靈隊長》\n攻擊力增加25%\n觸發技傷害增加175%\n普攻時，觸發「以自身攻擊力40%對目標造成傷害」\n必殺時，觸發「以自身攻擊力100%對目標造成傷害」\n\n《狙擊陣形》\n攻擊力增加100%\n造成治療量增加25%`
                },
                [SkillType.PASSIVE_1]: {
                    name: `追風烈撃`,
                    description: `必殺時，觸發「使目標必殺技傷害減少15%(2回合)」\n必殺時，觸發「以自身攻擊力80%對目標造成傷害」`
                },
                [SkillType.PASSIVE_2]: {
                    name: `腐蝕箭袋`,
                    description: `普攻時，觸發「使目標受到觸發技傷害增加20%(最多5層)」\n普攻時，觸發「以自身攻擊力35%對目標造成傷害」`
                },
                [SkillType.PASSIVE_3]: {
                    name: `強烈靈氣`,
                    description: `普攻時，觸發「使我方全體必殺技傷害增加6%(最多5層)」\n必殺時，觸發「使目標受到必殺技傷害增加15%(最多2層)」`
                },
                [SkillType.GENERAL_1]: {
                    name: `必殺+ (6潛)`,
                    description: `使自身必殺技傷害增加10%`
                },
                [SkillType.GENERAL_2]: {
                    name: `免疫麻痺 (12潛)`,
                    description: `使自身免疫麻痺`
                }
            },
            [Locale.sc]: {
                [SkillType.SKILL_S]: {
                    name: `精灵箭术。连连矢`,
                    description: `使自身造成伤害增加10/10/15/15/20%(3回合)，以自身攻击力200%对目标造成伤害，再使自身获得「攻击时，触发『以自身攻击力50/65/80/95/110%对目标造成伤害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_1]: {
                    name: `精灵箭术。连连矢`,
                    description: `使自身造成伤害增加10%(3回合)，以自身攻击力200%对目标造成伤害，再使自身获得「攻击时，触发『以自身攻击力50%对目标造成伤害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_2]: {
                    name: `精灵箭术。连连矢`,
                    description: `使自身造成伤害增加10%(3回合)，以自身攻击力200%对目标造成伤害，再使自身获得「攻击时，触发『以自身攻击力65%对目标造成伤害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_3]: {
                    name: `精灵箭术。连连矢`,
                    description: `使自身造成伤害增加15%(3回合)，以自身攻击力200%对目标造成伤害，再使自身获得「攻击时，触发『以自身攻击力80%对目标造成伤害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_4]: {
                    name: `精灵箭术。连连矢`,
                    description: `使自身造成伤害增加15%(3回合)，以自身攻击力200%对目标造成伤害，再使自身获得「攻击时，触发『以自身攻击力95%对目标造成伤害』(3回合)」，CD: 3`
                },
                [SkillType.SKILL_5]: {
                    name: `精灵箭术。连连矢`,
                    description: `使自身造成伤害增加20%(3回合)，以自身攻击力200%对目标造成伤害，再使自身获得「攻击时，触发『以自身攻击力110%对目标造成伤害』(3回合)」，CD: 3`
                },
                [SkillType.ATTACK]: {
                    name: `拉弦射撃`,
                    description: `以自身攻击力100%对目标造成伤害`
                },
                [SkillType.LEADER]: {
                    name: `深藏不露的胜负心`,
                    description: `我方全体HP增加20%\n我方全体攻击者丶妨碍者获得「队伍中至少有3名风属性角色时，发动《精灵队长》」\n我方全体获得「队伍中至少有2名火属性角色时，发动《狙击阵形》」\n\n《精灵队长》\n攻击力增加25%\n触发技伤害增加175%\n普攻时，触发「以自身攻击力40%对目标造成伤害」\n必杀时，触发「以自身攻击力100%对目标造成伤害」\n\n《狙击阵形》\n攻击力增加100%\n造成治疗量增加25%`
                },
                [SkillType.PASSIVE_1]: {
                    name: `追风烈撃`,
                    description: `必杀时，触发「使目标必杀技伤害减少15%(2回合)」\n必杀时，触发「以自身攻击力80%对目标造成伤害」`
                },
                [SkillType.PASSIVE_2]: {
                    name: `腐蚀箭袋`,
                    description: `普攻时，触发「使目标受到触发技伤害增加20%(最多5层)」\n普攻时，触发「以自身攻击力35%对目标造成伤害」`
                },
                [SkillType.PASSIVE_3]: {
                    name: `强烈灵气`,
                    description: `普攻时，触发「使我方全体必杀技伤害增加6%(最多5层)」\n必杀时，触发「使目标受到必杀技伤害增加15%(最多2层)」`
                },
                [SkillType.GENERAL_1]: {
                    name: `必杀+ (6潜)`,
                    description: `使自身必杀技伤害增加10%`
                },
                [SkillType.GENERAL_2]: {
                    name: `免疫麻痹 (12潜)`,
                    description: `使自身免疫麻痹`
                }
            },
            [Locale.en]: {
                [SkillType.SKILL_S]: {
                    name: `Arrow Storm`,
                    description: `Increase your Damage Output by 10/10/15/15/20% for 3 turns, damage the target (200% Attack Power), then gain the following ability for 3 turns: "On Attack, damage the target for 50/65/80/95/110% Attack Power". CD: 3`
                },
                [SkillType.SKILL_1]: {
                    name: `Arrow Storm`,
                    description: `Increase your Damage Output by 10% for 3 turns, damage the target (200% Attack Power), then gain the following ability for 3 turns: "On Attack, damage the target for 50% Attack Power". CD: 3`
                },
                [SkillType.SKILL_2]: {
                    name: `Arrow Storm`,
                    description: `Increase your Damage Output by 10% for 3 turns, damage the target (200% Attack Power), then gain the following ability for 3 turns: "On Attack, damage the target for 65% Attack Power". CD: 3`
                },
                [SkillType.SKILL_3]: {
                    name: `Arrow Storm`,
                    description: `Increase your Damage Output by 15% for 3 turns, damage the target (200% Attack Power), then gain the following ability for 3 turns: "On Attack, damage the target for 80% Attack Power". CD: 3`
                },
                [SkillType.SKILL_4]: {
                    name: `Arrow Storm`,
                    description: `Increase your Damage Output by 15% for 3 turns, damage the target (200% Attack Power), then gain the following ability for 3 turns: "On Attack, damage the target for 95% Attack Power". CD: 3`
                },
                [SkillType.SKILL_5]: {
                    name: `Arrow Storm`,
                    description: `Increase your Damage Output by 20% for 3 turns, damage the target (200% Attack Power), then gain the following ability for 3 turns: "On Attack, damage the target for 110% Attack Power". CD: 3`
                },
                [SkillType.ATTACK]: {
                    name: `Whirlwind Bow`,
                    description: `Damage the target (100% Attack Power).`
                },
                [SkillType.LEADER]: {
                    name: `Hidden Ambition`,
                    description: `Increase the party's HP by 20%.\n\nIf there are at least 3 Wind allies in your party, give all allied Attackers and Obstructors "Elf Captain".\nIf there are at least 2 Fire allies in your party, give the party "Sniper Formation".\n\n"Elf Captain"\nIncrease your Attack Power by 25%.\nIncrease damage from triggered abilities by 175%.\nOn Basic Attack, damage the target for 40% of your Attack Power.\nOn Ultimate Skill, damage the target for 100% of your Attack Power.\n\n"Sniper Formation"\nIncrease your Attack Power by 100%.\nIncrease the effect of your healing skills by 25%.`
                },
                [SkillType.PASSIVE_1]: {
                    name: `Hurricane Tomahawk`,
                    description: `On Ultimate Skill, reduce the target's Ultimate Skill Damage by 15% for 2 turns.\nOn Ultimate Skill, damage the target (80% Attack Power).`
                },
                [SkillType.PASSIVE_2]: {
                    name: `Frayed Pouch`,
                    description: `On Basic Attack, increase the target's Damage Taken from trigger attacks by 20% (Max 5 Stacks).\nOn Basic Attack, damage the target (35% Attack Power).`
                },
                [SkillType.PASSIVE_3]: {
                    name: `Strong Resolve`,
                    description: `On Basic Attack, increase the party's Ultimate Skill Damage by 6% (Max 5 Stacks).\nOn Ultimate Skill, increase the target's Damage Taken from Ultimate Skills by 15% (Max 2 Stacks).`
                },
                [SkillType.GENERAL_1]: {
                    name: `Ultimate Skill +`,
                    description: `Increase your Ultimate Skill Damage by 10%.`
                },
                [SkillType.GENERAL_2]: {
                    name: `Paralysis Immunity`,
                    description: `Gain immunity to Paralysis.`
                }
            },
            [Locale.jp]: {
                [SkillType.SKILL_S]: {
                    name: `エルフ弓術、連撃の矢`,
                    description: `自分の与えるダメージを10/10/15/15/20%増加させ(3ターン)、200%の攻撃力でターゲットにダメージを与え、自分が「攻撃時『自分の50/65/80/95/110%の攻撃力でターゲットにダメージを与える』を誘発する(３ターン)」を獲得する[CD:3]`
                },
                [SkillType.SKILL_1]: {
                    name: `エルフ弓術、連撃の矢`,
                    description: `自分の与えるダメージを10%増加させ(3ターン)、200%の攻撃力でターゲットにダメージを与え、自分が「攻撃時『自分の50%の攻撃力でターゲットにダメージを与える』を誘発する(３ターン)」を獲得する[CD:3]`
                },
                [SkillType.SKILL_2]: {
                    name: `エルフ弓術、連撃の矢`,
                    description: `自分の与えるダメージを10%増加させ(3ターン)、200%の攻撃力でターゲットにダメージを与え、自分が「攻撃時『自分の65%の攻撃力でターゲットにダメージを与える』を誘発する(３ターン)」を獲得する[CD:3]`
                },
                [SkillType.SKILL_3]: {
                    name: `エルフ弓術、連撃の矢`,
                    description: `自分の与えるダメージを15%増加させ(3ターン)、200%の攻撃力でターゲットにダメージを与え、自分が「攻撃時『自分の80%の攻撃力でターゲットにダメージを与える』を誘発する(３ターン)」を獲得する[CD:3]`
                },
                [SkillType.SKILL_4]: {
                    name: `エルフ弓術、連撃の矢`,
                    description: `自分の与えるダメージを15%増加させ(3ターン)、200%の攻撃力でターゲットにダメージを与え、自分が「攻撃時『自分の95%の攻撃力でターゲットにダメージを与える』を誘発する(３ターン)」を獲得する[CD:3]`
                },
                [SkillType.SKILL_5]: {
                    name: `エルフ弓術、連撃の矢`,
                    description: `自分の与えるダメージを20%増加させ(3ターン)、200%の攻撃力でターゲットにダメージを与え、自分が「攻撃時『自分の110%の攻撃力でターゲットにダメージを与える』を誘発する(３ターン)」を獲得する[CD:3]`
                },
                [SkillType.ATTACK]: {
                    name: `弓射撃`,
                    description: `自分の100%の攻撃力でターゲットにダメージを与える`
                },
                [SkillType.LEADER]: {
                    name: `内に秘めた勝負心`,
                    description: `味方全体のHPを20%増加させる\n味方のアタッカーとデバッファーが「チーム内に3人以上の風属性のキャラがいる時、《エルフ隊長》を発動する」を獲得する\n味方全体が「チーム内に2人以上の火属性のキャラがいる時、《狙撃陣形》」を発動する\n\n《エルフ隊長》\n攻撃力が25%増加する\n誘発スキルのダメージが175%増加する\n通常攻撃時「自分の40%の攻撃力でターゲットにダメージを与える」を誘発する\n必殺技攻撃時「自分の100%の攻撃力でターゲットにダメージを与える」を誘発する\n\n《狙撃陣形》\n攻撃力を100%増加させる\n治療をする時、回復HP量を25%増加させる`
                },
                [SkillType.PASSIVE_1]: {
                    name: `追風烈撃`,
                    description: `必殺技攻撃時「ターゲットの必殺技攻撃ダメージを15%減少させる(2ターン)」を誘発する\n必殺技攻撃時「自分の80%の攻撃力でターゲットにダメージを与える」を誘発する`
                },
                [SkillType.PASSIVE_2]: {
                    name: `腐食の矢筒`,
                    description: `通常攻撃時「ターゲットが受ける誘発スキルのダメージを20%増加させる(最高5スタック)」を誘発する\n通常攻撃時「自分の35%の攻撃力でターゲットにダメージを与える」を誘発する`
                },
                [SkillType.PASSIVE_3]: {
                    name: `霊気の強撃`,
                    description: `通常攻撃時「味方全体の必殺技ダメージを6%増加させる(最高5スタック)」を誘発する\n必殺技攻撃時「ターゲットが受ける必殺技ダメージを15%増加させる(最高2スタック)」を誘発する`
                },
                [SkillType.GENERAL_1]: {
                    name: `必殺＋`,
                    description: `自分の必殺技ダメージを10%増加させる`
                },
                [SkillType.GENERAL_2]: {
                    name: `麻痺無効`,
                    description: `自分を麻痺無効にする`
                }
            },
            [Locale.kr]: {
                [SkillType.SKILL_S]: {
                    name: `엘프궁술・연격의 화살`,
                    description: `자신이 가하는 데미지 10/10/15/15/20% 증가(3턴), 자신의 공격 데미지의 200%만큼 타깃에게 데미지를 가하고 자신은 「공격 시 『자신의 공격 데미지의 50/65/80/95/110%만큼 타깃에게 데미지』(3턴)」발동 [CD: 3]`
                },
                [SkillType.SKILL_1]: {
                    name: `엘프궁술・연격의 화살`,
                    description: `자신이 가하는 데미지 10% 증가(3턴), 자신의 공격 데미지의 200%만큼 타깃에게 데미지를 가하고 자신은 「공격 시 『자신의 공격 데미지의 50%만큼 타깃에게 데미지』(3턴)」발동 [CD: 3]`
                },
                [SkillType.SKILL_2]: {
                    name: `엘프궁술・연격의 화살`,
                    description: `자신이 가하는 데미지 10% 증가(3턴), 자신의 공격 데미지의 200%만큼 타깃에게 데미지를 가하고 자신은 「공격 시 『자신의 공격 데미지의 65%만큼 타깃에게 데미지』(3턴)」발동 [CD: 3]`
                },
                [SkillType.SKILL_3]: {
                    name: `엘프궁술・연격의 화살`,
                    description: `자신이 가하는 데미지 15% 증가(3턴), 자신의 공격 데미지의 200%만큼 타깃에게 데미지를 가하고 자신은 「공격 시 『자신의 공격 데미지의 80%만큼 타깃에게 데미지』(3턴)」발동 [CD: 3]`
                },
                [SkillType.SKILL_4]: {
                    name: `엘프궁술・연격의 화살`,
                    description: `자신이 가하는 데미지 15% 증가(3턴), 자신의 공격 데미지의 200%만큼 타깃에게 데미지를 가하고 자신은 「공격 시 『자신의 공격 데미지의 95%만큼 타깃에게 데미지』(3턴)」발동 [CD: 3]`
                },
                [SkillType.SKILL_5]: {
                    name: `엘프궁술・연격의 화살`,
                    description: `자신이 가하는 데미지 20% 증가(3턴), 자신의 공격 데미지의 200%만큼 타깃에게 데미지를 가하고 자신은 「공격 시 『자신의 공격 데미지의 110%만큼 타깃에게 데미지』(3턴)」발동 [CD: 3]`
                },
                [SkillType.ATTACK]: {
                    name: `활 사격`,
                    description: `자신의 공격 데미지의 100%만큼 타깃에게 데미지`
                },
                [SkillType.LEADER]: {
                    name: `숨길 수 없는 승부욕`,
                    description: `아군 전체 HP 20% 증가\n아군 딜러, 디스럽터는 「팀에 풍속성 캐릭터가 최소 3명 이상 있을 시 《엘프 대장》발동」획득\n아군 전체는「팀에 화속성 캐릭터가 최소 2명 이상 있을 시, 《저격 포메이션》발동」획득\n\n《엘프 대장》\n공격 데미지 25% 증가\n발동형 스킬 데미지 175% 증가\n일반 공격 시 「자신의 공격 데미지의 40%만큼 타깃에게 데미지」발동\n궁극기 발동 시 「자신의 공격 데미지의 100%만큼 타깃에게 데미지」발동\n\n《저격 포메이션》\n공격 데미지 100% 증가\n부여하는 치유량 25% 증가`
                },
                [SkillType.PASSIVE_1]: {
                    name: `추풍의 맹격`,
                    description: `궁극기 발동 시 「타깃의 궁극기 데미지 15% 감소(2턴)」발동\n궁극기 발동 시 「자신의 공격 데미지의 80%만큼 타깃에게 데미지」발동`
                },
                [SkillType.PASSIVE_2]: {
                    name: `부식 화살통`,
                    description: `일반 공격 시 「타깃이 받는 발동형 스킬 데미지 20% 증가(최대 5중첩)」발동\n일반 공격 시 「자신의 공격 데미지의 35% 만큼 타깃에게 데미지」발동`
                },
                [SkillType.PASSIVE_3]: {
                    name: `영기의 강격`,
                    description: `일반 공격 시 「아군 전체의 궁극기 데미지 6% 증가(최대 5중첩)」 발동\n궁극기 발동 시 「타깃이 받는 궁극기 데미지 15% 증가(최대 2중첩)」 발동`
                },
                [SkillType.GENERAL_1]: {
                    name: `궁극기+`,
                    description: `자신의 궁극기 데미지 10% 증가`
                },
                [SkillType.GENERAL_2]: {
                    name: `마비 면역`,
                    description: `자신에게 마비 면역 효과`
                }
            }
        }
    },
]