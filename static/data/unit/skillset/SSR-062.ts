import { DispatchSkillType, Locale, SkillType } from '@/plugins/utils/enums';
import { SkillSet } from '@/interface/unit/skillset';

export const SkillSet_10047: SkillSet[] =  [
    {
        version: 1,
        lastDate: "",
        remark: {
            [Locale.tc]: ``,
            [Locale.sc]: ``,
            [Locale.en]: ``,
            [Locale.jp]: ``,
            [Locale.kr]: ``
        },
        skill: {
            [Locale.tc]: {
                [SkillType.SKILL_S]: {
                    name: `暴風新星`,
                    description: `使目標受到傷害增加15/15/20/20/25%(最多1層)、再以自身攻擊力330/376/422/468/514%對目標造成傷害，CD:4`
                },
                [SkillType.SKILL_1]: {
                    name: `暴風新星`,
                    description: `使目標受到傷害增加15%(最多1層)、再以自身攻擊力330%對目標造成傷害，CD:4`
                },
                [SkillType.SKILL_2]: {
                    name: `暴風新星`,
                    description: `使目標受到傷害增加15%(最多1層)、再以自身攻擊力376%對目標造成傷害，CD:4`
                },
                [SkillType.SKILL_3]: {
                    name: `暴風新星`,
                    description: `使目標受到傷害增加20%(最多1層)、再以自身攻擊力422%對目標造成傷害，CD:4`
                },
                [SkillType.SKILL_4]: {
                    name: `暴風新星`,
                    description: `使目標受到傷害增加20%(最多1層)、再以自身攻擊力468%對目標造成傷害，CD:4`
                },
                [SkillType.SKILL_5]: {
                    name: `暴風新星`,
                    description: `使目標受到傷害增加25%(最多1層)、再以自身攻擊力514%對目標造成傷害，CD:4`
                },
                [SkillType.ATTACK]: {
                    name: `風刃`,
                    description: `以自身攻擊力100%對目標造成傷害`
                },
                [SkillType.LEADER]: {
                    name: `風之逸才`,
                    description: `我方風屬性角色攻擊力增加80%\n我方風屬性角色普攻傷害增加60%\n我方風屬性角色必殺技傷害增加30%`
                },
                [SkillType.PASSIVE_1]: {
                    name: `暴風！臣服於我！`,
                    description: `必殺技傷害增加20%\n必殺時，觸發「以自身攻擊力50%對目標造成傷害」`
                },
                [SkillType.PASSIVE_2]: {
                    name: `狂風障壁`,
                    description: `防禦時，觸發「使我方全體受到傷害減少20%(1回合)」`
                },
                [SkillType.PASSIVE_3]: {
                    name: `魔法劍士`,
                    description: `造成傷害增加10%\n必殺時，觸發「以自身攻擊力50%對目標造成傷害」\n每經過4回合，觸發「使敵方全體受到風屬性傷害增加25%(1回合)」`
                },
                [SkillType.GENERAL_1]: {
                    name: `攻擊+`,
                    description: `使自身攻擊力增加10%`
                },
                [SkillType.GENERAL_2]: {
                    name: `免疫麻痺`,
                    description: `使自身免疫麻痺`
                }
            },
            [Locale.sc]: {
                [SkillType.SKILL_S]: {
                    name: `暴风新星`,
                    description: `使目标受到伤害增加15/15/20/20/25%(最多1层)丶再以自身攻击力330/376/422/468/514%对目标造成伤害，CD:4`
                },
                [SkillType.SKILL_1]: {
                    name: `暴风新星`,
                    description: `使目标受到伤害增加15%(最多1层)丶再以自身攻击力330%对目标造成伤害，CD:4`
                },
                [SkillType.SKILL_2]: {
                    name: `暴风新星`,
                    description: `使目标受到伤害增加15%(最多1层)丶再以自身攻击力376%对目标造成伤害，CD:4`
                },
                [SkillType.SKILL_3]: {
                    name: `暴风新星`,
                    description: `使目标受到伤害增加20%(最多1层)丶再以自身攻击力422%对目标造成伤害，CD:4`
                },
                [SkillType.SKILL_4]: {
                    name: `暴风新星`,
                    description: `使目标受到伤害增加20%(最多1层)丶再以自身攻击力468%对目标造成伤害，CD:4`
                },
                [SkillType.SKILL_5]: {
                    name: `暴风新星`,
                    description: `使目标受到伤害增加25%(最多1层)丶再以自身攻击力514%对目标造成伤害，CD:4`
                },
                [SkillType.ATTACK]: {
                    name: `风刃`,
                    description: `以自身攻击力100%对目标造成伤害`
                },
                [SkillType.LEADER]: {
                    name: `风之逸才`,
                    description: `我方风属性角色攻击力增加80%\n我方风属性角色普攻伤害增加60%\n我方风属性角色必杀技伤害增加30%`
                },
                [SkillType.PASSIVE_1]: {
                    name: `暴风！臣服於我！`,
                    description: `必杀技伤害增加20%\n必杀时，触发「以自身攻击力50%对目标造成伤害」`
                },
                [SkillType.PASSIVE_2]: {
                    name: `狂风障壁`,
                    description: `防御时，触发「使我方全体受到伤害减少20%(1回合)」`
                },
                [SkillType.PASSIVE_3]: {
                    name: `魔法剑士`,
                    description: `造成伤害增加10%\n必杀时，触发「以自身攻击力50%对目标造成伤害」\n每经过4回合，触发「使敌方全体受到风属性伤害增加25%(1回合)」`
                },
                [SkillType.GENERAL_1]: {
                    name: `攻击+`,
                    description: `使自身攻击力增加10%`
                },
                [SkillType.GENERAL_2]: {
                    name: `免疫麻痹`,
                    description: `使自身免疫麻痹`
                }
            },
            [Locale.en]: {
                [SkillType.SKILL_S]: {
                    name: `Blizzard Star`,
                    description: `Increase the target's Damage Taken by 15/15/20/20/25% (Max 1 Stack), and damage the target (330/376/422/468/514% Attack Power). CD: 4`
                },
                [SkillType.SKILL_1]: {
                    name: `Blizzard Star`,
                    description: `Increase the target's Damage Taken by 15% (Max 1 Stack), and damage the target (330% Attack Power). CD: 4`
                },
                [SkillType.SKILL_2]: {
                    name: `Blizzard Star`,
                    description: `Increase the target's Damage Taken by 15% (Max 1 Stack), and damage the target (376% Attack Power). CD: 4`
                },
                [SkillType.SKILL_3]: {
                    name: `Blizzard Star`,
                    description: `Increase the target's Damage Taken by 20% (Max 1 Stack), and damage the target (422% Attack Power). CD: 4`
                },
                [SkillType.SKILL_4]: {
                    name: `Blizzard Star`,
                    description: `Increase the target's Damage Taken by 20% (Max 1 Stack), and damage the target (468% Attack Power). CD: 4`
                },
                [SkillType.SKILL_5]: {
                    name: `Blizzard Star`,
                    description: `Increase the target's Damage Taken by 25% (Max 1 Stack), and damage the target (514% Attack Power). CD: 4`
                },
                [SkillType.ATTACK]: {
                    name: `Wind Dagger`,
                    description: `Damage the target (100% Attack Power).`
                },
                [SkillType.LEADER]: {
                    name: `Rabbit from a Hat`,
                    description: `Increase Wind allies' Attack Power by 80%.\nIncrease Wind allies' Basic Attack Damage by 60%.\nIncrease Wind allies' Ultimate Skill Damage by 30%.`
                },
                [SkillType.PASSIVE_1]: {
                    name: `Blizzard! Obey Me!`,
                    description: `Increase your Ultimate Skill Damage by 20%.\nOn Ultimate Skill, damage the target (50% Attack Power).`
                },
                [SkillType.PASSIVE_2]: {
                    name: `Gale Shield`,
                    description: `When Guarding, reduce the party's Damage Taken by 20% for 1 turn.`
                },
                [SkillType.PASSIVE_3]: {
                    name: `Battlemage`,
                    description: `Increase your Damage Output by 10%.\nOn Ultimate Skill, damage the target (50% Attack Power).\nAfter every four turns, increase all enemies' Damage Taken from Wind attacks by 25% for 1 turn.`
                },
                [SkillType.GENERAL_1]: {
                    name: `Attack+`,
                    description: `Increase your Attack Power by 10%.`
                },
                [SkillType.GENERAL_2]: {
                    name: `Paralysis Immunity`,
                    description: `Gain immunity to Paralysis.`
                }
            },
            [Locale.jp]: {
                [SkillType.SKILL_S]: {
                    name: `暴風之星`,
                    description: `ターゲットが受けるダメージを15/15/20/20/25%増加させ(最高1スタック)、自分の330/376/422/468/514%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_1]: {
                    name: `暴風之星`,
                    description: `ターゲットが受けるダメージを15%増加させ(最高1スタック)、自分の330%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_2]: {
                    name: `暴風之星`,
                    description: `ターゲットが受けるダメージを15%増加させ(最高1スタック)、自分の376%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_3]: {
                    name: `暴風之星`,
                    description: `ターゲットが受けるダメージを20%増加させ(最高1スタック)、自分の422%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_4]: {
                    name: `暴風之星`,
                    description: `ターゲットが受けるダメージを20%増加させ(最高1スタック)、自分の468%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_5]: {
                    name: `暴風之星`,
                    description: `ターゲットが受けるダメージを25%増加させ(最高1スタック)、自分の514%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.ATTACK]: {
                    name: `風之刃`,
                    description: `自分の100%の攻撃力でターゲットにダメージを与える`
                },
                [SkillType.LEADER]: {
                    name: `風之逸材`,
                    description: `風属性の味方の攻撃力が80%増加する\n風属性の味方の通常攻撃ダメージが60%増加する\n風属性の味方の必殺技ダメージが30%増加する`
                },
                [SkillType.PASSIVE_1]: {
                    name: `暴れる風よ、屈服せよ！`,
                    description: `必殺技ダメージが20%増加する\n必殺技攻撃時「自分の50%の攻撃力でターゲットにダメージを与える」を誘発する`
                },
                [SkillType.PASSIVE_2]: {
                    name: `狂風障壁`,
                    description: `防禦時「味方全体の受けるダメージを20%減少させる(1ターン)」を誘発する`
                },
                [SkillType.PASSIVE_3]: {
                    name: `魔導剣士`,
                    description: `与えるダメージが10%増加する\n必殺技攻撃時「自分の50%の攻撃力でターゲットにダメージを与える」を誘発する\n4ターン毎に「敵全体の風属性から受けるダメージを25%増加させる(1ターン)」を誘発する`
                },
                [SkillType.GENERAL_1]: {
                    name: `攻撃+`,
                    description: `自分の攻撃力を10%増加させる`
                },
                [SkillType.GENERAL_2]: {
                    name: `麻痺無効`,
                    description: `自分を麻痺無効にする`
                }
            },
            [Locale.kr]: {
                [SkillType.SKILL_S]: {
                    name: `폭풍 신성`,
                    description: `타깃이 받는 데미지 15/15/20/20/25%증가 (최대 1중첩), 자신 공격 데미지의 330/376/422/468/514%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_1]: {
                    name: `폭풍 신성`,
                    description: `타깃이 받는 데미지 15%증가 (최대 1중첩), 자신 공격 데미지의 330%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_2]: {
                    name: `폭풍 신성`,
                    description: `타깃이 받는 데미지 15%증가 (최대 1중첩), 자신 공격 데미지의 376%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_3]: {
                    name: `폭풍 신성`,
                    description: `타깃이 받는 데미지 20%증가 (최대 1중첩), 자신 공격 데미지의 422%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_4]: {
                    name: `폭풍 신성`,
                    description: `타깃이 받는 데미지 20%증가 (최대 1중첩), 자신 공격 데미지의 468%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_5]: {
                    name: `폭풍 신성`,
                    description: `타깃이 받는 데미지 25%증가 (최대 1중첩), 자신 공격 데미지의 514%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.ATTACK]: {
                    name: `바람의 칼`,
                    description: `자신 공격 데미지의 100%만큼 타깃에게 데미지`
                },
                [SkillType.LEADER]: {
                    name: `바람의 인재`,
                    description: `아군 풍속성 캐릭터의 공격 데미지 80% 증가\n아군 풍속성 캐릭터의 일반 공격 데미지 60% 증가\n아군 풍속성 캐릭터의 궁극기 데미지 30% 증가`
                },
                [SkillType.PASSIVE_1]: {
                    name: `폭풍이여, 내게 복종하라!`,
                    description: `궁극기 데미지 20%증가\n궁극기 발동 시, [자신의 공격 데미지 50%만큼 타깃에게 데미지] 효과 발동`
                },
                [SkillType.PASSIVE_2]: {
                    name: `광풍의 장벽`,
                    description: `방어 시, [아군 전체가 받는 데미지 20% 감소(1턴)] 효과 발동`
                },
                [SkillType.PASSIVE_3]: {
                    name: `마법 검사`,
                    description: `가하는 데미지 10% 증가\n궁극기 발동 시, [자신의 공격 데미지 50%만큼 타깃에게 데미지] 효과 발동\n4턴마다, [적 전체가 받는 풍속성 데미지 25%증가 (1턴)] 효과 발동`
                },
                [SkillType.GENERAL_1]: {
                    name: `공격+`,
                    description: `자신의 공격 데미지 10% 증가`
                },
                [SkillType.GENERAL_2]: {
                    name: `마비 면역`,
                    description: `자신에게 마비 면역 효과`
                }
            }
        }
    },
]