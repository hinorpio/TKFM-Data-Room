import { DispatchSkillType, Locale, SkillType } from '@/plugins/utils/enums';
import { SkillSet } from '@/interface/unit/skillset';

export const SkillSet_10147: SkillSet[] =  [
    {
        version: 1,
        lastDate: "",
        remark: {
            [Locale.tc]: ``,
            [Locale.sc]: ``,
            [Locale.en]: ``,
            [Locale.jp]: ``,
            [Locale.kr]: ``
        },
        skill: {
            [Locale.tc]: {
                [SkillType.SKILL_S]: {
                    name: `只好把你做成料理了！`,
                    description: `以自身攻擊力330/376%對目標造成傷害，使目標受到風、光屬性傷害增加30/35%(2回合)。CD:4\n\n3絆+:使目標受到風、光屬性傷害增加40/45/50%(2回合)，並使目標受到傷害增加15%(2回合)，再以自身攻擊力422/468/514%對目標造成傷害。CD:4`
                },
                [SkillType.SKILL_1]: {
                    name: `只好把你做成料理了！`,
                    description: `以自身攻擊力330%對目標造成傷害，使目標受到風、光屬性傷害增加30%(2回合)。CD:4`
                },
                [SkillType.SKILL_2]: {
                    name: `只好把你做成料理了！`,
                    description: `以自身攻擊力376%對目標造成傷害，使目標受到風、光屬性傷害增加35%(2回合)。CD:4`
                },
                [SkillType.SKILL_3]: {
                    name: `只好把你做成料理了！`,
                    description: `使目標受到風、光屬性傷害增加40%(2回合)，並使目標受到傷害增加15%(2回合)，再以自身攻擊力422%對目標造成傷害。CD:4`
                },
                [SkillType.SKILL_4]: {
                    name: `只好把你做成料理了！`,
                    description: `使目標受到風、光屬性傷害增加45%(2回合)，並使目標受到傷害增加15%(2回合)，再以自身攻擊力468%對目標造成傷害。CD:4`
                },
                [SkillType.SKILL_5]: {
                    name: `只好把你做成料理了！`,
                    description: `使目標受到風、光屬性傷害增加50%(2回合)，並使目標受到傷害增加15%(2回合)，再以自身攻擊力514%對目標造成傷害。CD:4`
                },
                [SkillType.ATTACK]: {
                    name: `不浪費食物斬`,
                    description: `以自身攻擊力100%對目標造成傷害`
                },
                [SkillType.LEADER]: {
                    name: `魔物料理大廚`,
                    description: `我方全體最大HP增加40%\n我方全體攻擊力增加50%\n使自身獲得《大家都來吃吃看吧！》\n\n《大家都來吃吃看吧！》\n攻擊力增加100%\n造成傷害增加50%\n必殺技傷害增加100%\n普攻傷害增加100%\n自身必殺時，觸發「使自身以外的我方全體獲得《秘製・魔物手路菜》」\n\n《秘製・魔物手路菜》\n攻擊力增加100%(2回合)\n造成傷害增加30%(2回合)\n必殺技傷害增加50%(1回合)\n普攻傷害增加100%(2回合)\n行動後，觸發「清除自身以外的我方全體《秘製・魔物手路菜》的所有效果」(2回合)(觸發1次後解除)`
                },
                [SkillType.PASSIVE_1]: {
                    name: `慢工細活`,
                    description: `必殺時，觸發「使自身以外的我方全體獲得《享受大餐》」\n\n《享受大餐》\n攻擊力增加100%(2回合)\n普攻傷害增加100%(2回合)\n行動後，觸發「清除自身以外的我方全體《享受大餐》的所有效果」(2回合)(觸發1次後解除)`
                },
                [SkillType.PASSIVE_2]: {
                    name: `應急用配菜`,
                    description: `防禦時，觸發「使自身以外的我方全體獲得《醃製內臟乾》」\n\n《醃製內臟乾》\n行動後，觸發「使自身受到傷害減少20%(1回合)，並清除自身以外的我方全體《醃製內臟乾》的效果」(1回合)(觸發1次後解除)`
                },
                [SkillType.PASSIVE_3]: {
                    name: `刀工指導`,
                    description: `使我方站位2獲得「必殺時，觸發『《魔物肢解》』」\n\n《魔物肢解》\n使目標受到必殺技傷害增加100%(1回合)\n使目標被治療時回復量減少20%(4回合)`
                },
                [SkillType.GENERAL_1]: {
                    name: `攻擊+`,
                    description: `使自身攻擊力增加10%`
                },
                [SkillType.GENERAL_2]: {
                    name: `免疫麻痺`,
                    description: `使自身免疫麻痺`
                }
            },
            [Locale.sc]: {
                [SkillType.SKILL_S]: {
                    name: `只好把你做成料理了！`,
                    description: `以自身攻击力330/376%对目标造成伤害，使目标受到风丶光属性伤害增加30/35%(2回合)。CD:4\n\n3绊+:使目标受到风丶光属性伤害增加40/45/50%(2回合)，并使目标受到伤害增加15%(2回合)，再以自身攻击力422/468/514%对目标造成伤害。CD:4`
                },
                [SkillType.SKILL_1]: {
                    name: `只好把你做成料理了！`,
                    description: `以自身攻击力330%对目标造成伤害，使目标受到风丶光属性伤害增加30%(2回合)。CD:4`
                },
                [SkillType.SKILL_2]: {
                    name: `只好把你做成料理了！`,
                    description: `以自身攻击力376%对目标造成伤害，使目标受到风丶光属性伤害增加35%(2回合)。CD:4`
                },
                [SkillType.SKILL_3]: {
                    name: `只好把你做成料理了！`,
                    description: `使目标受到风丶光属性伤害增加40%(2回合)，并使目标受到伤害增加15%(2回合)，再以自身攻击力422%对目标造成伤害。CD:4`
                },
                [SkillType.SKILL_4]: {
                    name: `只好把你做成料理了！`,
                    description: `使目标受到风丶光属性伤害增加45%(2回合)，并使目标受到伤害增加15%(2回合)，再以自身攻击力468%对目标造成伤害。CD:4`
                },
                [SkillType.SKILL_5]: {
                    name: `只好把你做成料理了！`,
                    description: `使目标受到风丶光属性伤害增加50%(2回合)，并使目标受到伤害增加15%(2回合)，再以自身攻击力514%对目标造成伤害。CD:4`
                },
                [SkillType.ATTACK]: {
                    name: `不浪费食物斩`,
                    description: `以自身攻击力100%对目标造成伤害`
                },
                [SkillType.LEADER]: {
                    name: `魔物料理大厨`,
                    description: `我方全体最大HP增加40%\n我方全体攻击力增加50%\n使自身获得《大家都来吃吃看吧！》\n\n《大家都来吃吃看吧！》\n攻击力增加100%\n造成伤害增加50%\n必杀技伤害增加100%\n普攻伤害增加100%\n自身必杀时，触发「使自身以外的我方全体获得《秘制·魔物手路菜》」\n\n《秘制·魔物手路菜》\n攻击力增加100%(2回合)\n造成伤害增加30%(2回合)\n必杀技伤害增加50%(1回合)\n普攻伤害增加100%(2回合)\n行动後，触发「清除自身以外的我方全体《秘制·魔物手路菜》的所有效果」(2回合)(触发1次後解除)`
                },
                [SkillType.PASSIVE_1]: {
                    name: `慢工细活`,
                    description: `必杀时，触发「使自身以外的我方全体获得《享受大餐》」\n\n《享受大餐》\n攻击力增加100%(2回合)\n普攻伤害增加100%(2回合)\n行动後，触发「清除自身以外的我方全体《享受大餐》的所有效果」(2回合)(触发1次後解除)`
                },
                [SkillType.PASSIVE_2]: {
                    name: `应急用配菜`,
                    description: `防御时，触发「使自身以外的我方全体获得《腌制内脏乾》」\n\n《腌制内脏乾》\n行动後，触发「使自身受到伤害减少20%(1回合)，并清除自身以外的我方全体《腌制内脏乾》的效果」(1回合)(触发1次後解除)`
                },
                [SkillType.PASSIVE_3]: {
                    name: `刀工指导`,
                    description: `使我方站位2获得「必杀时，触发『《魔物肢解》』」\n\n《魔物肢解》\n使目标受到必杀技伤害增加100%(1回合)\n使目标被治疗时回复量减少20%(4回合)`
                },
                [SkillType.GENERAL_1]: {
                    name: `攻击+`,
                    description: `使自身攻击力增加10%`
                },
                [SkillType.GENERAL_2]: {
                    name: `免疫麻痹`,
                    description: `使自身免疫麻痹`
                }
            },
            [Locale.en]: {
                [SkillType.SKILL_S]: {
                    name: `Dish Transformation!`,
                    description: `Damage the target (330/376% Attack Power). Increase the target's Damage Taken from Wind and Light attacks by 30/35% for 2 turns. CD:4\n\nLv3+: Increase the target's Damage Taken from Wind and Light attacks by 40/45/50% for 2 turns and increase the target's Damage Taken by 15% for 2 turns. Then damage the target (422/468/514% Attack Power). CD:4`
                },
                [SkillType.SKILL_1]: {
                    name: `Dish Transformation!`,
                    description: `Damage the target (330% Attack Power). Increase the target's Damage Taken from Wind and Light attacks by 30% for 2 turns. CD:4`
                },
                [SkillType.SKILL_2]: {
                    name: `Dish Transformation!`,
                    description: `Damage the target (376% Attack Power). Increase the target's Damage Taken from Wind and Light attacks by 35% for 2 turns. CD:4`
                },
                [SkillType.SKILL_3]: {
                    name: `Dish Transformation!`,
                    description: `Increase the target's Damage Taken from Wind and Light attacks by 40% for 2 turns and increase the target's Damage Taken by 15% for 2 turns. Then damage the target (422% Attack Power). CD:4`
                },
                [SkillType.SKILL_4]: {
                    name: `Dish Transformation!`,
                    description: `Increase the target's Damage Taken from Wind and Light attacks by 45% for 2 turns and increase the target's Damage Taken by 15% for 2 turns. Then damage the target (468% Attack Power). CD:4`
                },
                [SkillType.SKILL_5]: {
                    name: `Dish Transformation!`,
                    description: `Increase the target's Damage Taken from Wind and Light attacks by 50% for 2 turns and increase the target's Damage Taken by 15% for 2 turns. Then damage the target (514% Attack Power). CD:4`
                },
                [SkillType.ATTACK]: {
                    name: `Food Saving Slash`,
                    description: `Damage the target (100% Attack Power).`
                },
                [SkillType.LEADER]: {
                    name: `Monster Cuisine Chef`,
                    description: `Increase the party's max HP by 40%.\nIncrease the party's Attack Power by 50%.\nGain yourself "Come and Try!".\n\n"Come and Try!"\nIncrease Attack Power by 100%.\nIncrease Damage Output by 50%.\nIncrease Ultimate Skill Power by 100%.\nIncrease Basic Attack Power by 100%.\nOn Ultimate Skill, trigger the following effect: other allies gain "Secret Recipe: Beast Cuisine".\n\n"Secret Recipe: Beast Cuisine"\nIncrease Attack Power by 100% for 2 turns.\nIncrease Damage Output by 30% for 2 turns.\nIncrease Ultimate Skill Power by 50% for 1 turn.\nIncrease Basic Attack Power by 100% for 2 turns.\nAfter performing an action, trigger the following effect: remove all "Secret Recipe: Beast Cuisine" effects from other allies for 2 turns (removes after triggering once).`
                },
                [SkillType.PASSIVE_1]: {
                    name: `Precise Work`,
                    description: `On Ultimate Skill, trigger the following effect: other allies gain "Enjoy the Feast".\n\n"Enjoy the Feast"\nIncrease Attack Power by 100% for 2 turns.\nIncrease Basic Attack Power by 100% for 2 turns.\nAfter performing an action, trigger the following effect: remove all "Enjoy the Feast" effects from other allies for 2 turns (removes after triggering once).`
                },
                [SkillType.PASSIVE_2]: {
                    name: `Emergency Prep`,
                    description: `When in Guard Stance, trigger the following effect: other allies gain "Marinated Dried Organs".\n\n"Marinated Dried Organs"\nAfter performing an action, trigger the following effect: decrease your Damage Taken by 20% for 1 turn and remove all "Marinated Dried Organs" effects from other allies for 1 turn (removes after triggering once).`
                },
                [SkillType.PASSIVE_3]: {
                    name: `Knife Guidance`,
                    description: `The 2nd-position ally gains the following effect: On Ultimate Skill, trigger "Beast Dissection".\n\n"Beast Dissection"\nIncrease the target's Damage Taken from Ultimate Skills by 100% for 1 turn.\nDecrease the target's Recovery Rate by 20% for 4 turns.`
                },
                [SkillType.GENERAL_1]: {
                    name: `Attack+`,
                    description: `Increase your Attack Power by 10%.`
                },
                [SkillType.GENERAL_2]: {
                    name: `Paralysis Immunity`,
                    description: `Gain immunity to Paralysis`
                }
            },
            [Locale.jp]: {
                [SkillType.SKILL_S]: {
                    name: `あなたを料理するしかないね！`,
                    description: `自分の330/376%の攻撃力でターゲットにダメージを与え、ターゲットが風、光属性から受けるダメージを30/35%増加させる(2ターン)[CD:4]\n\nLv3+: ターゲットが風、光属性から受けるダメージを40/45/50%増加させ(2ターン)、ターゲットが受けるダメージを15%増加させ(2ターン)、自分の422/468/514%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_1]: {
                    name: `あなたを料理するしかないね！`,
                    description: `自分の330%の攻撃力でターゲットにダメージを与え、ターゲットが風、光属性から受けるダメージを30%増加させる(2ターン)[CD:4]`
                },
                [SkillType.SKILL_2]: {
                    name: `あなたを料理するしかないね！`,
                    description: `自分の376%の攻撃力でターゲットにダメージを与え、ターゲットが風、光属性から受けるダメージを35%増加させる(2ターン)[CD:4]`
                },
                [SkillType.SKILL_3]: {
                    name: `あなたを料理するしかないね！`,
                    description: `ターゲットが風、光属性から受けるダメージを40%増加させ(2ターン)、ターゲットが受けるダメージを15%増加させ(2ターン)、自分の422%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_4]: {
                    name: `あなたを料理するしかないね！`,
                    description: `ターゲットが風、光属性から受けるダメージを45%増加させ(2ターン)、ターゲットが受けるダメージを15%増加させ(2ターン)、自分の468%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.SKILL_5]: {
                    name: `あなたを料理するしかないね！`,
                    description: `ターゲットが風、光属性から受けるダメージを50%増加させ(2ターン)、ターゲットが受けるダメージを15%増加させ(2ターン)、自分の514%の攻撃力でターゲットにダメージを与える[CD:4]`
                },
                [SkillType.ATTACK]: {
                    name: `お粗末ダメ斬り`,
                    description: `自分の100%の攻撃力でターゲットにダメージを与える`
                },
                [SkillType.LEADER]: {
                    name: `魔物料理のシェフ`,
                    description: `味方全体の最大HPが40%増加する\n味方全体の攻撃力が50%増加する\n自分が《みんな食べてみて！》を獲得する\n\n《みんな食べてみて！》\n攻撃力が100%増加する\n与えるダメージが50%増加する\n必殺技ダメージが100%増加する\n通常攻撃ダメージが100%増加する\n自分が必殺技攻撃時「自分以外の味方全体が《秘伝・魔物得意料理》を獲得する」を誘発する\n\n《秘伝・魔物得意料理》\n攻撃力が100%増加する(2ターン)\n与えるダメージが30%増加する(2ターン)\n必殺技ダメージが50%増加する(1ターン)\n通常攻撃ダメージが100%増加する(2ターン)\n行動後「自分以外の味方全体の《秘伝・魔物得意料理》による全ての効果を消す」を誘発する(2ターン)(誘発1回後に解除)`
                },
                [SkillType.PASSIVE_1]: {
                    name: `丹精込めた料理`,
                    description: `必殺技攻撃時「自分以外の味方全体が《ごちそう満喫》を獲得する」を誘発する\n\n《ごちそう満喫》\n攻撃力が100%増加する(2ターン)\n通常攻撃ダメージが100%増加する(2ターン)\n行動後「自分以外の味方全体の《ごちそう満喫》による全ての効果を消す」を誘発する(2ターン)(誘発1回後に解除)`
                },
                [SkillType.PASSIVE_2]: {
                    name: `いざというの時のおかず`,
                    description: `防御時「自分以外の味方全体が《内臓の干物》を獲得する」を誘発する\n\n《内臓の干物》\n行動後「自分が受けるダメージを20%減少させ(1ターン)、自分以外の味方全体の《内臓の干物》の効果を消す」を誘発する(1ターン)(誘発1回後に解除)`
                },
                [SkillType.PASSIVE_3]: {
                    name: `包丁さばきの指導`,
                    description: `2番目の味方が「必殺技攻撃時《魔物解体》を誘発する」を獲得する\n\n《魔物解体》\nターゲットが受ける必殺技ダメージを100%増加させる(1ターン)\nターゲットが治療を受ける時、回復HP量を20%減少させる(4ターン)`
                },
                [SkillType.GENERAL_1]: {
                    name: `攻撃+`,
                    description: `自分の攻撃力を10%増加させる`
                },
                [SkillType.GENERAL_2]: {
                    name: `麻痺無効`,
                    description: `自分を麻痺無効にする`
                }
            },
            [Locale.kr]: {
                [SkillType.SKILL_S]: {
                    name: `널 요리해 버리는 수밖에!`,
                    description: `자신의 공격 데미지의 330/376%만큼 타깃에게 데미지, 타깃이 받는 풍속성, 광속성 데미지 30/35% 증가(2턴) [CD: 4]\n\nLv3+: 타깃이 받는 풍속성, 광속성 데미지 40/45/50% 증가(2턴), 타깃이 받는 데미지 15% 증가(2턴), 다시 자신의 공격 데미지의 422/468/514%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_1]: {
                    name: `널 요리해 버리는 수밖에!`,
                    description: `자신의 공격 데미지의 330%만큼 타깃에게 데미지, 타깃이 받는 풍속성, 광속성 데미지 30% 증가(2턴) [CD: 4]`
                },
                [SkillType.SKILL_2]: {
                    name: `널 요리해 버리는 수밖에!`,
                    description: `자신의 공격 데미지의 376%만큼 타깃에게 데미지, 타깃이 받는 풍속성, 광속성 데미지 35% 증가(2턴) [CD: 4]`
                },
                [SkillType.SKILL_3]: {
                    name: `널 요리해 버리는 수밖에!`,
                    description: `타깃이 받는 풍속성, 광속성 데미지 40% 증가(2턴), 타깃이 받는 데미지 15% 증가(2턴), 다시 자신의 공격 데미지의 422%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_4]: {
                    name: `널 요리해 버리는 수밖에!`,
                    description: `타깃이 받는 풍속성, 광속성 데미지 45% 증가(2턴), 타깃이 받는 데미지 15% 증가(2턴), 다시 자신의 공격 데미지의 468%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.SKILL_5]: {
                    name: `널 요리해 버리는 수밖에!`,
                    description: `타깃이 받는 풍속성, 광속성 데미지 50% 증가(2턴), 타깃이 받는 데미지 15% 증가(2턴), 다시 자신의 공격 데미지의 514%만큼 타깃에게 데미지 [CD: 4]`
                },
                [SkillType.ATTACK]: {
                    name: `음식을 남기면 못 써`,
                    description: `자신의 공격 데미지의 100%만큼 타깃에게 데미지`
                },
                [SkillType.LEADER]: {
                    name: `마물 요리 셰프`,
                    description: `아군 전체의 최대 HP 40% 증가\n아군 전체의 공격 데미지 50% 증가\n자신은 《모두 어서 먹어보라니까!》 획득\n\n《모두 어서 먹어보라니까!》\n공격 데미지 100% 증가\n가하는 데미지 50% 증가\n궁극기 데미지 100% 증가\n일반 공격 데미지 100% 증가\n자신의 궁극기 발동 시 「자신 이외의 아군 전체는 《특제・마물 요리》획득」 트리거\n\n《특제・마물 요리》\n공격 데미지 100% 증가(2턴)\n가하는 데미지 30% 증가(2턴)\n궁극기 데미지 50% 증가(1턴)\n일반 공격 데미지 100% 증가(2턴)\n행동 후 「자신 이외의 아군 전체의 《특제・마물 요리》의 모든 효과 제거」트리거(2턴)(1회 트리거 후 해제)`
                },
                [SkillType.PASSIVE_1]: {
                    name: `신중한 작업`,
                    description: `궁극기 발동 시 「자신 이외의 아군 전체는 《즐거운 만찬》획득」 트리거\n\n《즐거운 만찬》\n공격 데미지 100% 증가(2턴)\n일반 공격 데미지 100% 증가(2턴)\n행동 후 「자신 이외의 아군 전체의 《즐거운 만찬》의 모든 효과 제거」트리거(2턴)(1회 트리거 후 해제)`
                },
                [SkillType.PASSIVE_2]: {
                    name: `비상용 반찬`,
                    description: `방어 시 「자신 이외의 아군 전체는 《말린 내장 요리》획득」 트리거\n\n《말린 내장 요리》\n행동 후 「자신의 받는 데미지 20% 감소(1턴), 자신 이외의 아군 전체의 《말린 내장 요리》의 효과 제거」트리거(1턴)(1회 트리거 후 해제)`
                },
                [SkillType.PASSIVE_3]: {
                    name: `칼질 지도`,
                    description: `2번 자리 아군은 「궁극기 발동 시 『《마물 해체》』트리거」획득\n\n《마물 해체》\n타깃이 받는 궁극기 데미지 100% 증가(1턴)\n타깃이 치유를 받을 시 회복량 20% 감소(4턴)`
                },
                [SkillType.GENERAL_1]: {
                    name: `공격+`,
                    description: `자신의 공격 데미지 10% 증가`
                },
                [SkillType.GENERAL_2]: {
                    name: `마비 면역`,
                    description: `자신에게 마비 면역`
                }
            }
        }
    },
]